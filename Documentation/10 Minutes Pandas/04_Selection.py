import pandas as pd
import numpy as np
df = pd.read_csv("Housing.csv")

#Selection
#1. GETITEM []
print(df['area']) #or df.area,  both are same but first is more appropriate
print(df['area'].sort_values())  #Selection with sorting
print(df[['area', 'price']])  #Multiple selection
print(df[:2], "\n")  #Selection using slicinig, select two rows

#2. Selection by label: loc or at
print(('Selection by label:'))
print(df.loc[1, 'price'])
print(df.loc[0, ['price', 'area']]) #type:ignore
print(df.loc[df['price']>10000000])

#3. Selection by position: iloc and iat
print("\nSelection by Position:")
print(df.iloc[1]) #select rows by index
print(df.iloc[0:3, 1:2])  #select rows and columns both first will be for select rows and second will be for column
print(df.iloc[[1, 5, 2], [3, 7]]) #Select selective rows using direct index
print(df.iloc[1, 1])  #To getting a specific value
print(df.iat[1, 5])  #iat is only for singular valuee

#4. Boolean Indexing
print("\nBoolean Indexing:")
print(df[df['price'] > 2000000])  #Select rows in which price is greater than 2000000
print(df[df['basement'].isin(['yes'])])  #Select rows in which basement is present

